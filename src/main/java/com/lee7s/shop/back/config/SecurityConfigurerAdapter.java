...
import com.lee7s.shop.back.handler.security.LogoutSuccessHandler;
import com.lee7s.shop.back.handler.security.RestfulAccessDeniedHandler;
import com.lee7s.shop.back.handler.security.UnAuthEntryPoint;
import com.lee7s.shop.back.handler.security.filter.TokenAuthFilter;
import com.lee7s.shop.back.handler.security.filter.TokenLoginFilter;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                ...
                .antMatchers(
                        ...
                        "/back/sysDict/requestDictByParent/**",
                        "/back/sysDict/requestDictByCode/**",
                        "/back/sysDict/requestDictByParentAndSonCode/**",
                        "/",
                        "/favicon.ico",
                        "/static/**",
        ...
        http.logout().logoutUrl("/user/logout").addLogoutHandler(new LogoutSuccessHandler(jwtToken)); // 登出逻辑和处理器以及如果需要删除cookie

        // 登录过滤器
        http.addFilter(new TokenLoginFilter(authenticationManager(), jwtToken, userService));

        // 处理权限的过滤器
        http.addFilter(new TokenAuthFilter(authenticationManager(), jwtToken));
