...
import com.lee7s.shop.back.config.auto.EmailLee7sProperties;
import com.lee7s.shop.back.config.auto.EmailOrderProperties;
...
public class EmailCodeClient {
    ...
    @Autowired
    private EmailLee7sProperties emailLee7sProperties; // 商家属性

    @Autowired
    private EmailOrderProperties emailOrderProperties; // 客户属性


    public R sendEmailToLee7s(String email, String moneyToken){
        SimpleMailMessage message = new SimpleMailMessage();
        // 加昵称版本
//        message.setFrom(new InternetAddress(MimeUtility.encodeText(formEmail)).toString());
        message.setFrom(emailLee7sProperties.getFormEmailNickName() + "<" + emailLee7sProperties.getFormEmail() + ">");
        // 不加昵称版本
        // message.setFrom(formEmail);
        message.setTo(email);
        message.setSubject(emailLee7sProperties.getSubject());
        message.setText(emailLee7sProperties.getSendContentPrefix() + email + "," + emailLee7sProperties.getMoneyTokenPrefix() + moneyToken);
        // message.setCc("抄送人");
        // message.setBcc("密送人");
        try{
            javaMailSender.send(message);
            // 成功返回数据
            return R.ok(REnum.EMAIL_SEND_TO_LEE7S_SUCCESS.getStatusCode(),
                    REnum.EMAIL_SEND_TO_LEE7S_SUCCESS.getStatusMsg());
        }catch (Exception e) {
            e.printStackTrace();
            // 失败返回错误信息
            return R.error(REnum.EMAIL_SEND_TO_LEE7S_FAIL.getStatusCode(),
                    REnum.EMAIL_SEND_TO_LEE7S_FAIL.getStatusMsg());
        }
    }


    public R sendEmailOrder(String email, String orderSn){
        SimpleMailMessage message = new SimpleMailMessage();
        // 加昵称版本
//        message.setFrom(new InternetAddress(MimeUtility.encodeText(formEmail)).toString());
        message.setFrom(emailOrderProperties.getFormEmailNickName() + "<" + emailOrderProperties.getFormEmail() + ">");
        // 不加昵称版本
        // message.setFrom(formEmail);
        message.setTo(email);
        message.setSubject(emailOrderProperties.getSubject());
        message.setText(emailOrderProperties.getSendOrderSnContentPrefix() + orderSn);
        // message.setCc("抄送人");
        // message.setBcc("密送人");
        try{
            javaMailSender.send(message);
            // 成功返回数据
            return R.ok(REnum.EMAIL_SEND_ORDER_SUCCESS.getStatusCode(),
                    REnum.EMAIL_SEND_ORDER_SUCCESS.getStatusMsg());
        }catch (Exception e) {
            e.printStackTrace();
            // 失败返回错误信息
            return R.error(REnum.EMAIL_SEND_ORDER_FAIL.getStatusCode(),
                    REnum.EMAIL_SEND_ORDER_FAIL.getStatusMsg());
        }
    }
